...
- `group.html` + `js/group.js` — group dashboard, fully wired: fetches `/api/groups/[code]` (lowercase codes normalized), loading + page-error states (bad code / 404 / network), expenses table with two-step inline delete ("Sure?" arms 3s — never browser dialogs) and edit (form refills incl. paisa→rupee conversion, PUT on save, cancel back to add mode), add form with equal/exact split UI (rupee input → integer paisa at the edge), balances + who-pays-whom rendered from the server-computed values; every mutation re-fetches so balances never go stale. Mock data deleted
...
- Money travels as integer minor units (`amountCents`, `shareCents`, `balanceCents`) in every API payload; display goes through one `formatMoney(cents, currency)` helper — never format anywhere else. Display rules (Karim's review round, 2026-08-18): symbol BEFORE the number with NO space (`Rs4,200`); positive amounts green (`--green`), negative red (brightened `--error`); balances/settlements read as sentences ("Karim is owed", "Ali pays Sana") — natural word spacing, no flex gaps between the words.
- Currency symbols (Karim's review round, 2026-08-18): PKR Rs, USD $, GBP £, EUR €, CAD C$, **AED Dh, SAR SR** — AED/SAR do have native symbols (د.إ, ﷼) but they're Arabic-script glyphs that render as broken boxes in Latin fonts like DM Mono; Dh/SR are the common Latin forms. Swapping in the real glyphs is a one-line change in `js/group.js` if Karim ever wants it.
...
- [x] Review round 1 (2026-08-18, Karim's live test): group delete (API `DELETE /api/groups/[code]` + two-step UI button, removes code from localStorage, cascade proved live), Copy-link button (full group URL), AED→Dh / SAR→SR symbols (native glyphs break in Latin fonts), symbol-before-number with no space, brighter red + green for positive amounts, balances/settlements as sentences (flex gaps were splitting the words). 49/49 tests + delete probe green
